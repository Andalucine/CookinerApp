...
| POST | `/imports/wine` | `url`, `notebook_id` opcional | Lee la ficha de un vino en una tienda o bodega (schema.org Product + pistas del nombre y la descripción): vista previa `wine` lista para POST `/wines` (con `source_name` y `source_price` si la tienda publica el precio, `price_range` calculado de él y `pairing_notes` rellenado con las reglas de maridaje del tipo), con `warnings` (`no_product_data`, `no_name`, `no_type`, `no_winery`). 403 si el cuaderno es gratuito (sesión 8). |
