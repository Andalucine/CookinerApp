/** One note in the list: title, the start of its text and the day it was last changed. */
...
export function NoteCard({ note, onPress }: { note: NoteSummary; onPress: () => void }) {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={[note.title, note.preview, changed, by].filter(Boolean).join(", ")}
      onPress={onPress}
